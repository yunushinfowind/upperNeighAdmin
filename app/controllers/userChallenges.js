...
let getTotalPracticeDuration = async (userId,type) => {
    let practices = [];
    if(type == 'today'){
       practices = await Practice.findAll({ where: {  
        [Op.and]:
        [ { user_id : userId },
             {created_at : { 
                [Op.gt]: TODAY_START,
        ...
        practices = await Practice.findAll({ where: { [Op.and]:
            [ { user_id : userId },
                 {created_at : { 
                    [Op.gt]:year,
                  [Op.lt]: new Date()
...
let getPracticeSessionCount = async (userId,type) => {
    var session = 0;
       if(type == 'today'){
        session = await Practice.count({ where: {  
         [Op.and]:
         [ { user_id : userId },
              {created_at : { 
                  [Op.gt]: TODAY_START,
    ...
    session = await Practice.count({ where: { [Op.and]:
        [ { user_id : userId },
             {created_at : { 
                [Op.gt]:year,
              [Op.lt]: new Date() }}
...
let getPracticeFocusAvg = async (userId,type) => {
    var focus = 0;
        if(type == 'today'){
            focus = await Practice.findAll({where:{
                [Op.and]:
                [ { user_id : userId },
                     {created_at : { 
                         [Op.gt]: TODAY_START,
    ...
    focus = await Practice.findAll({ where: { [Op.and]:
        [ { user_id : userId },
             {created_at : { 
                [Op.gt]:year,
              [Op.lt]: new Date() }}
...
let getPracticeMoodAvg =async (userId,type) => {
    var mood = 0;
        if(type == 'today'){
            mood = await Practice.findAll({where:{[Op.and]:
            [ { user_id:userId } ,
              {created_at : {
                 [Op.gt]:TODAY_START,
                [Op.lt]: new Date() }}
            ...
            focus = await Practice.findAndCountAll({ where: {[Op.and]:
            [ { user_id:userId},
              {created_at: {
                [Op.gt]: MON,
                [Op.lt]: new Date() }}
...
let getPracticeProductivityAvg = async (userId,type) => {
    var productivity = 0;
    if(type == 'today'){
        productivity = await Practice.findAll({where:{ [Op.and]:
            [ { user_id : userId },
                 {created_at : { 
                     [Op.gt]: TODAY_START,
                     [Op.lt]: new Date()
    ...
    try{
             var All = [];
                ...
                if (isValidToekn) {
                        ...
                        var list = await Userchallenge.findAndCountAll({  where:
                        //  {  
                        //      [Op.and]: 
                                      { user_id:loginId },
...
exports.likeVideo = async (req,res) => {
    try{
        ...
        if(req.query.type == '0'){
            const likes = await Challengelikes.destroy({ where : { user_id : loginId , 
                challenge_id : req.query.challenge_id }});
...
exports.commentDetails = async(req,res) => {
    try{ let token = await User.getToken(req);
        let isValidToekn = await validateToekn(token);
        var All = [];
        if (isValidToekn) {
            let loginId = await getLoginUserId(token);
...
exports.commentLike = async (req,res) => {
    ...
    try{
        ...
        if (isValidToekn) {
            ...
            if(req.query.type == '1'){
                const likes = await Commentlikes.destroy({ where : { user_id : loginId , 
                    comment_id : req.query.comment_id }});
    ...
    try{
        ...
        if(isValidToekn){
            ...
            if(req.query.type == 'unlike'){
                const likes = await Practicelikes.destroy({ where : { user_id : loginId , 
                    practice_id : req.query.practice_id }});
...
exports.practiceCommentList = async (req,res) => {
    try{
        ...
        if(isValidToekn){
            ...
            let list = await Practicecomment.findAndCountAll({  where: {
                [Op.and] : [
                    { user_id : loginId },
                    {practice_id: req.query.practice_id}
                ]
...
exports.practicecommentLike = async(req,res) => {
    try{
        ...
        if(isValidToekn){
            ...
            if(req.query.type == 'unlike'){
                const likes = await PracticelikeonComment.destroy({ where : { user_id : loginId , 
                    practice_comment_id : req.query.practice_comment_id }});
